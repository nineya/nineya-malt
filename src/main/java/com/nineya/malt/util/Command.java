...
public class Command {
    ...
    private static void compress(){
        ...
        if (FileTool.isFile(path)){     //是文件
            ...
            List<FileType> fileTypes = Config.getCfg().getFileTypes();
            boolean copy = Config.getCfg().isNoCompressCopy();
    ...
    private static void config(){
        Log.info("当前配置信息：");
        Log.info(Console.print(Config.asMap()));
    }
    ...
    private static void compressFile(FileType fileType, String path, String newPath) {
        ...
        switch (fileType){
            case HTML:{
                Config.getCfg().getEnter().compressHtml();
                break;
            }
        }
