...
public class FileTool {
    ...
    public static Map readYaml(String path) {
        ...
        try {
            ...
            Log.info("配置文件不存在，将载入默认配置！");
            Log.info("如要修改配置，请将配置文件命名为“application.yml”，放入jar同级目录下。");
            return null;
        }
    }
            ...
            if (fs != null && fs.length != 0){
                for (File f : fs){
                    if (f.isDirectory()){   //文件夹
                        ...
                        Log.error("错误：未知错误文件(" + f.getAbsolutePath() + ")");
                    }
                }
            }
        }
    ...
    public static String jarWholePath(){
        String jarWholePath = Config.class.getProtectionDomain().getCodeSource().getLocation().getFile();
        try {
